...
class Order:
    ...
    def post_reqest_order(self, payload):
        response = requests.post(f"{Urls.BASE_URL}{Urls.ORDER_URL}", json=payload)
    ...
    def put_reqest_take_orders(self, id_order, id_courier):
        response = requests.put(f"{Urls.BASE_URL}{Urls.ORDER_URL}/accept/{id_order}?courierId={id_courier}")
        return response.status_code, response.json()

    @allure.step("принять заказ без трека")
    def put_reqest_take_orders_without_track(self, id_courier):
        response = requests.put(f"{Urls.BASE_URL}{Urls.ORDER_URL}/accept/?courierId={id_courier}")
        return response.status_code, response.json()

    @allure.step("принять заказ без id  курьера")
    def put_reqest_take_orders_without_id_courier(self, id_order):
        response = requests.put(f"{Urls.BASE_URL}{Urls.ORDER_URL}/accept/{id_order}")
        return response.status_code, response.json()

    @allure.step("получить данные заказа")
    def get_reqest_get_order(self, track):
        response = requests.get(f"{Urls.BASE_URL}{Urls.ORDER_URL}/track?t={track}")
        return response.status_code, response.json()
